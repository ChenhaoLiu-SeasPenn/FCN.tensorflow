...
def main(argv=None):
    with tf.device('/device:GPU:0'):
        ...
        if FLAGS.pos_weight == 1:
            ...
            loss = tf.reduce_mean((tf.nn.weighted_cross_entropy_with_logits(targets = tf.one_hot(tf.squeeze(annotation, squeeze_dims=[3], dtype=tf.int32), FLAGS.class_num, axis=-1),
                                                                              logits=logits,
                                                                              pos_weight=weights,
                                                                              name="entropy")))
